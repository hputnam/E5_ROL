...
path.p<-"~/MyProjects/E5_ROL/RAnalysis/Data/20191023/" #the location of all your respirometry files 
...
Sample.Info <- read.csv(file="~/MyProjects/E5_ROL/RAnalysis/Data/20191023_PI_Curve_Sample_Info.csv", header=T) #read sample.info data
...
#view rates
Photo.R
write.csv(Photo.R,"~/MyProjects/E5_ROL/RAnalysis/Output/20191023_PI_Curve_rates.csv")
...
PA <- subset(Data, Species=="Apulchra")
...
#####
# #Plot curves
# pdf("~/MyProjects/E5_ROL/RAnalysis/Output/20191023_PICurves.pdf")
# par(mfrow=c(1,1))
# 
# #Pocillopora acuta
...
curve.nlslrc.PA = nls(Pc ~ (1/(2*theta))*(AQY*PAR+Am-sqrt((AQY*PAR+Am)^2-4*AQY*theta*Am*PAR))-Rd,
                      start=list(Am=(max(Pc)-min(Pc)),AQY=0.05,Rd=-min(Pc),theta=0.001)) 
...
pdf("~/MyProjects/E5_ROL/RAnalysis/Output/20191023_NLLSR_PICurves.pdf")
par(mfrow=c(1,1))
...
write.csv(PI.Output.PA,"~/MyProjects/E5_ROL/RAnalysis/Output/20191023_PI_Curve_Output.csv")
